...
FREE_MODELS = [
    ...
    {"provider": "groq", "model": "llama-3.1-8b-instant", "priority": 4},
    {"provider": "openrouter", "model": "google/gemini-2.5-flash", "priority": 5},
    {"provider": "openrouter", "model": "deepseek/deepseek-chat", "priority": 6},
    {"provider": "openrouter", "model": "openai/gpt-4o-mini", "priority": 7},
]
...
class ModelRotationService:
    ...
    @staticmethod
    def get_next_available_free_model(db: Session, current_provider: str | None = None, current_model: str | None = None) -> dict:
        """
        ...
        for m in FREE_MODELS:
            ...
            if db_status:
                ...
                if db_status.is_exhausted:
                    ...
                    if exhausted_until is not None:
                        if exhausted_until.tzinfo is None and compare_now.tzinfo is not None:
                            compare_now = compare_now.replace(tzinfo=None)
                        elif exhausted_until.tzinfo is not None and compare_now.tzinfo is None:
            ...
            if not is_exhausted:
                ...
                is_current = (m["provider"] == current_provider and m["model"] == current_model)
        ...
        if not available_candidates:
            logger.warning("Todos los modelos gratuitos están en cooldown. Utilizando el de prioridad superior.")
    ...
    @staticmethod
    def mark_model_exhausted(db: Session, provider: str, model: str, reason: str, cooldown_seconds: int | None = None) -> datetime:
        """
        ...
        if not cooldown_seconds:
            ...
            if "decommissioned" in reason_lower or "not found" in reason_lower or "not supported" in reason_lower or "400" in reason_lower:
                # Bloqueo por 30 días para modelos descontinuados o inexistentes
                cooldown_seconds = 30 * 24 * 3600
            elif "429" in reason or "rate_limit" in reason_lower or "limit reached" in reason_lower or "too many requests" in reason_lower:
                cooldown_seconds = 60
            else:
        ...
        db_status = db.query(FreeModelStatus).filter(FreeModelStatus.id == model_id).first()
        if not db_status:
    ...
    @staticmethod
    def track_usage_and_check_limits(db: Session, provider: str, model: str, input_tokens: int, output_tokens: int):
        """
        ...
        if quota:
            ...
            if rpd_limit and db_status.requests_used_today >= rpd_limit:
                ...
                db_status.exhausted_reason = "Supera límite proactivo diario de peticiones (RPD)"
                logger.warning("Modelo '%s' superó proactivamente su cuota diaria. Cooldown hasta %s", model_id, tomorrow.isoformat())
                ...
                if db_status.is_exhausted:
                    ...
                    if exhausted_until is not None:
                        ...
                        if exhausted_until > compare_now:
                            is_exhausted = True
                            cooldown_left_seconds = int((exhausted_until - compare_now).total_seconds())
